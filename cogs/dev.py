...
class Dev(discord.Cog):
    ...
    dev = discord.SlashCommandGroup(name='dev', description='Developer restricted commands.')
    set = dev.create_subgroup(name='set')

    @set.command(name='name', description='Изменить имя любого хряка')
    async def set_name(
            self, ctx: PiggyContext,
            name: discord.Option(str, max_length=32),
            new_name: discord.Option(str, max_length=20)
    ):
    ...
    @dev.command(name='announce', description='ANNOUNCE TO ALL SERVERS, should be used extremely rarely.')
